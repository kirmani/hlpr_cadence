...
const char *clusterRostopic = "/beliefs/clusters";//"/beliefs/clusters";
...
cluster_cb (const hlpr_segmentation::SegmentedClusters& msg)
{
    ...
    //std::cout << "Clusters received" << std::endl;    
    ...
    //std::cout << "Normals received" << std::endl;
    ...
    //std::cout << "Plane received" << std::endl;
...
main (int argc, char **argv)
{
  ...
  HlprFeatureExtraction multi_plane_app;
 ...
 // planeSub = nh->subscribe(planeRostopic, 1, plane_cb);
  ...
  while (!interrupt && (!pA.viz || !viewer->wasStopped ()))
  {
    ...
    if(cloud_mutex.try_lock())
    {
      ...
      // std::cout << "Out of process once" << std::endl;
     // pub.publish(feats);
      cloud_mutex.unlock();
    }
   ...
   // std::cout << "Selected cluster hue: " << feats[selected_cluster_index].hue << std::endl;
    ...
    for(int i = 0; i < feats.size(); i++) {
      ...
      objectPoseTF(rosMsg.transforms[i],i);
    }
    //std::cout << "filled ros message, publishing" << std::endl;
    pub.publish(rosMsg);
    

    //std::cout << "Get the indices of the cluster points from the selected cluster" << std::endl;
   ...
   // std::cout << "Removes the clusters from the point cloud" << std::endl;
    ...
    // std::cout << "Publish the point cloud with the selected object removed" << std::endl;
